...
Begin visualizing trends seen in the camera trap data. First, connect to
the database and load the tables and packages from before. We will also
be using `ggplot2`.
...
To use the same color scheme shown, run
`devtools::install_github("cdanielcadena/tanagR")` and load the package
`tanagR`.
...
We want to know the number of individuals in each species detected in
September and October. There are many species observed throughout these
data, so we will break down the detections as we go.
...
Let's visualize this for species with more than 100 detections in
September or October.

```{r 100_detect_months, message = FALSE}
...
```{r vis_100_detect_months, eval = FALSE, message = FALSE}
...
```{r plot_vis_100_detect_months, echo = FALSE}
knitr::include_graphics("figs/vis_100_detect_months-1.png")
```

### Carnivora order detections

Focusing on the order Carnivora, we want to visualize the number of
individuals in each species detected in September and October.

```{r carnivora_months, message = FALSE}
carnivora <- sp_month[sp_month$order == "Carnivora", ] %>% # select species in order Carnivora
  group_by(month, common_name) %>% 
  summarize(n_individuals = sum(group_number)) # calculate number of individuals detected per species per month
```

```{r vis_carnivora_months, eval = FALSE, message = FALSE}
...
```{r plot_vis_carnivora_months, echo = FALSE}
knitr::include_graphics("figs/vis_carnivora_months-1.png")
```

## Detections across orders

Many detections were only identified to the order level. Rather than
looking at the species level, we want to visualize the number of
individuals in each order detected in September and October.

```{r order_months, message = FALSE}
...
```{r vis_order_months, eval = FALSE, message = FALSE}
...
```{r plot_vis_order_months, echo = FALSE}
knitr::include_graphics("figs/vis_order_months-1.png")
```

## Detections over time

We want to visualize wildlife trends over time in a couple of ways.
Let's break this down again to help understand the bigger picture.
...
We want to visualize the number of individuals detected over time and
...
```{r vis_wlf_time, eval = FALSE, message = FALSE}
...
```{r plot_vis_wlf_time, echo = FALSE}
knitr::include_graphics("figs/vis_wlf_time-1.png")
```

### Wildlife and human-domestic detections over time

We want to visualize the number of individuals detected over time, along
with the number of humans and domestic animals. We will first look at
the year level.
...
```{r vis_wlf_human_time, eval = FALSE, message = FALSE}
...
```{r plot_vis_wlf_human_time, echo = FALSE}
knitr::include_graphics("figs/vis_wlf_human_time-1.png")
```
...
```{r vis_wlf_human_julian, eval = FALSE, message = FALSE}
...
```{r plot_vis_wlf_human_julian, echo = FALSE}
knitr::include_graphics("figs/vis_wlf_human_julian-1.png")
```

Surprisingly, wildlife detections do not appear to decrease as human and
domestic animal detections increase.
